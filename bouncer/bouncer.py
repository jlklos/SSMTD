...
#here we tell it to do the decorated method when the root of the web server is requested
@app.route('/', methods=['POST'])
def bounce():
	#here we use a nested funtion to keep the namespace clean
	#this funciton is used by the thread to send data on to the next host.
	...
	#(otherwise the host would have to wait until the final node returned OK so that each host down the line
	#could then send OK back to the next host etc.)
	t= threading.Thread(target=nextbounce, args=(request.json, random.choice(hosts)))
	#start the thread
	t.start()
