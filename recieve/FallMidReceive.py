...
class initialize:
	...
	def __init__(self, port):
        	 self.port = port   #Read port argument from main

	def first_socket(self):
    		#Opens a socket to communicate the buffer length before beginning to listen to the main chunks.
		sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
		self.port = 7789  #Nah bruh
		sock.bind(("0.0.0.0", self.port))
		sock.listen(5)
		print("Listening on " + str(self.port) + " for a buffer length...")
...
class reassemble():
	...
	#print parts
        ...
        for part in xrange(len(parts)):
            #print part
            #print parts[part]
            split = parts[part].split()
            slices[split[1]] = base64.b64decode(split[0].strip())
        parts = slices
	print parts


        fp = open(self.fileToRec, 'wb')
        # Openin g the file to receive
...
def main():
    ...
    """
    size = initialize(args.port) 
    chunksize = size.first_socket()
    print chunksize
    parts = listener(args.port, chunksize)
    parts.listening()
